...
import ProgressionChart from '@/components/swimmers/ProgressionChart';
...
interface PersonalBestsProps {
   ...
   */
  fetchPersonalBests?: (swimmerId: string) => Promise<SwimmerPersonalBests>;
  fetchResults?: (swimmerId: string) => Promise<CompetitionResult[]>;
  /** Shown instead of rendering nothing when the swimmer has no times yet. */
  emptyMessage?: string;
}
...
export default function PersonalBests({
  swimmerId,
  fetchPersonalBests = getSwimmerPersonalBests,
  fetchResults = getSwimmerResults,
  emptyMessage,
}: PersonalBestsProps) {
  const { formatDate } = useFormatters();
  const { locale } = useClubRegion();
  const [data, setData] = useState<SwimmerPersonalBests | null>(null);
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.all([fetchPersonalBests(swimmerId), fetchResults(swimmerId)])
  ...
  }, [swimmerId, fetchPersonalBests, fetchResults]);
