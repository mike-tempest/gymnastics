    ...
    return (
      <MainLayout>
        <div className="min-h-dvh bg-canvas p-6 sm:p-10">
          <div className="max-w-7xl mx-auto">
            {memberError ? (
              <ErrorState message={memberError} onRetry={refetchMember} />
            ) : (
              <EmptyState
                icon={Users}
      ...
      <div className="min-h-dvh bg-canvas p-6 sm:p-10">
        <div className="max-w-7xl mx-auto">
              ...
              <div className="flex items-center space-x-5">
                ...
                <div>
                  ...
                  <p className="text-grey-600 text-lg">
                    {calculateAge(member.dob)} years old • {formatGender(member.gender)}
                  </p>
                </div>
              </div>
            </div>
          ...
          {(error || memberError) && (
            <div className="mb-8">
              <ErrorState
                message={error || memberError || 'Something went wrong. Please try again.'}
                onRetry={() => {
                  setError(null);
                  refetchMember();
                }}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            ...
            <div className="bg-dark-primary rounded-3xl shadow-lg p-6 border border-white/20">
              ...
              <p className="text-white text-xl font-bold">{formatGender(member.gender)}</p>
            </div>
            ...
            <div className="bg-dark-primary rounded-3xl shadow-lg p-6 border border-white/20">
              ...
              <p className="text-white text-xl font-bold">
                {member.registration_number ? (
                  <>
                    {member.governing_body && (
                      <span className="block text-sm font-semibold text-text-secondary">
                        {GOVERNING_BODY_LABELS[member.governing_body]}
                      </span>
                    )}
                    {member.registration_number}
                  </>
          ...
          <div className="bg-dark-primary rounded-3xl shadow-lg p-6 border border-white/20 mb-8">
            ...
            <p className="text-white/60 text-lg">
              {member.emergency_contact || 'No emergency contact recorded'}
            </p>
          </div>
          ...
          <div className="bg-dark-primary rounded-3xl shadow-lg p-6 border border-white/20 mb-8">
            ...
            <p className="text-white/60 text-lg whitespace-pre-wrap">
              {member.medical_notes || 'No medical notes recorded'}
            </p>
          </div>
          ...
          {/* Attendance History */}
          <div className="bg-dark-primary rounded-3xl shadow-lg p-6 border border-white/20">
            <AttendanceHistory memberId={params.id} />
          </div>
        </div>
      </div>
