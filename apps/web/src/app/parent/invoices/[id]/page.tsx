            ...
            <div className="bg-dark-primary rounded-3xl p-4 md:p-6 shadow-lg border border-white/10">
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <p className="text-white font-medium tabular-nums">{formatDate(invoice.issued_date)}</p>
                </div>
                <div>
            ...
            <div className="bg-dark-primary rounded-3xl p-4 md:p-6 shadow-lg border border-white/10">
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <p className="text-white font-medium">{invoice.family?.primary_contact_name || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-text-tertiary text-xs mb-1">Email</p>
                  <p className="text-white font-medium break-all">{invoice.family?.primary_contact_email || 'N/A'}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-dark-primary rounded-3xl p-4 md:p-6 shadow-lg border border-white/10">
              ...
              <div className="space-y-3">
                <div className="flex justify-between">
                  ...
                  <span className="text-white font-medium tabular-nums">{formatCurrency(invoice.subtotal, invoice.currency)}</span>
                </div>
                {taxApplied && (
                  <div className="flex justify-between">
                    ...
                    <span className="text-white font-medium tabular-nums">{formatCurrency(invoice.tax_amount, invoice.currency)}</span>
                  </div>
                )}
                <div className="pt-3 border-t border-white/10">
                  <div className="flex justify-between items-center">
                    <span className="text-white font-semibold">Total</span>
                    <span className="font-serif text-2xl text-brand tabular-nums">{formatCurrency(invoice.total_amount, invoice.currency)}</span>
                  </div>
                </div>
              </div>
            </div>
                  ...
                  <div className="md:hidden space-y-3">
                    {invoice.items.map((item) => (
                      <div key={item.item_id} className="bg-white/5 rounded-2xl p-4 space-y-2">
                        ...
                        <div className="flex items-center justify-between text-sm">
                          <span className="text-text-tertiary tabular-nums">Qty: {item.quantity}</span>
                          <span className="text-text-tertiary tabular-nums">{formatCurrency(item.unit_price, invoice.currency)} each</span>
                        </div>
                        <div className="flex items-center justify-between pt-2 border-t border-white/10">
                          <span className="text-text-tertiary text-sm">Line total</span>
                          <span className="text-white font-semibold tabular-nums">{formatCurrency(item.total, invoice.currency)}</span>
                        </div>
                      </div>
                    ))}
                    <div className="bg-white/5 rounded-2xl p-4 space-y-3 mt-4">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-white font-semibold tabular-nums">{formatCurrency(invoice.subtotal, invoice.currency)}</span>
                      </div>
                      {taxApplied && (
                        <div className="flex items-center justify-between">
                          <span className="text-text-tertiary text-sm">
                      ...
                      <div className="flex items-center justify-between pt-3 border-t border-white/10">
                        ...
                        <span className="font-serif text-brand text-xl tabular-nums">{formatCurrency(invoice.total_amount, invoice.currency)}</span>
                      </div>
                    </div>
                  </div>
                      ...
                      <tbody>
                        {invoice.items.map((item) => (
                          <tr key={item.item_id} className="border-b border-white/10 last:border-0">
                            ...
                            <td className="py-4 text-white text-center tabular-nums">{item.quantity}</td>
                            <td className="py-4 text-white text-right tabular-nums">{formatCurrency(item.unit_price, invoice.currency)}</td>
                            <td className="py-4 text-white text-right font-semibold tabular-nums">{formatCurrency(item.total, invoice.currency)}</td>
                          </tr>
                        ))}
                      </tbody>
                      <tfoot>
                        <tr className="border-t-2 border-white/10">
                          <td colSpan={3} className="pt-4 text-white font-semibold text-right">Subtotal</td>
                          <td className="pt-4 text-white font-semibold text-right tabular-nums">{formatCurrency(invoice.subtotal, invoice.currency)}</td>
                        </tr>
                        {taxApplied && (
                          <tr>
                            ...
                            <td className="pt-2 text-white text-right tabular-nums">{formatCurrency(invoice.tax_amount, invoice.currency)}</td>
                          </tr>
                        )}
                        <tr className="border-t border-white/10">
                          <td colSpan={3} className="pt-3 text-white font-bold text-right">Total amount</td>
                          <td className="pt-3 text-brand text-xl font-bold text-right font-serif tabular-nums">{formatCurrency(invoice.total_amount, invoice.currency)}</td>
                        </tr>
                      </tfoot>
                    </table>
                  </div>
          ...
          <div className="bg-dark-primary rounded-3xl shadow-lg border border-white/10 mb-8">
            ...
            <div className="p-4 md:p-6">
              {invoice.status === 'paid' ? (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-white/70 text-sm tabular-nums">
                        {formatCurrency(invoice.total_amount, invoice.currency)} paid on {formatDate(invoice.updated_at)}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col items-center justify-center py-8 text-center">
                  <svg className="w-12 h-12 text-text-tertiary mb-4" fill="none" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewBox="0 0 24 24" stroke="currentColor">
                    <path d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  <p className="text-white font-semibold mb-1">No payments recorded</p>
                  <p className="text-text-secondary text-sm">Payments will appear here once processed.</p>
                </div>
              )}
            </div>
          </div>
