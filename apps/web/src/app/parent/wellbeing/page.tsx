      ...
      try {
        ...
        const enriched = await Promise.all(
          memberData.map(async (member) => {
            const todayLog = await fetchTodayCheckIn(member.member_id).catch(() => null);
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="bg-brand/5 border border-brand/20 rounded-2xl p-6 space-y-4">
          ...
          <div className="space-y-3 text-sm text-text-secondary leading-relaxed">
            <p>
              A quick daily check-in helps coaches understand how each {MEMBER_NOUN_LOWER} is
              feeling, so they can adapt sessions without needing to ask personal questions.
            </p>
            <p>
              Coaches only ever see a green, amber, or red indicator. They never see individual
              scores, private notes, or any cycle tracking data.
            </p>
            <p>
              Research shows that young {MEMBER_NOUN_PLURAL_LOWER}, particularly girls, are more
              likely to stay engaged in the sport when they feel supported and understood. Wellbeing
              tracking makes that possible without any awkward conversations.
            </p>
          </div>
        </div>
      </div>
