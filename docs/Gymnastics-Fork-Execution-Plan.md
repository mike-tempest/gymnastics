...
**Companion to:** *Forking Swimly for Gymnastics — Build Plan*
...
NGB verification, brand/name, and founding-club recruitment all run **in parallel** and only need to *land* before the phase that consumes them. If a task isn't on that chain, it can slip without moving the launch date. Guard the chain; be relaxed about the rest.
...
## Phase 0 — Decide & de-risk *(this week — ~3–5 days)*
...
## Phase 1 — Fork skeleton *(walking skeleton — the big one)*
...
*Say the word and I'll start on action 2 now — none of it needs the brand decision.*
