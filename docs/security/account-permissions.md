...
Verification covers every primary role on account creation/deletion and own/other account edits, restricted field injection, scoped reads and mutations, foreign family references, missing tenant context, inactive accounts, stale roles and token-club mismatch. Repository unit tests use a predicate-aware in-memory store. The account-access integration suite additionally uses disposable PostgreSQL, real JWT authentication and request tenant context to verify account operations, cross-club refusals and password-change session invalidation. It runs with the existing password-recovery integration configuration and does not use live accounts or providers. Deployed synthetic acceptance must be recorded separately before closure.
