...
test.describe('SwimNexus UK - Full User Journey Smoke Test', () => {
  ...
  test('Complete user journey: Login -> View members -> Create family -> Add member to squad -> Take attendance -> View invoices', async ({
    page,
  }) => {
    // ========================================
    // STEP 1: LOGIN
    ...
    const hasMembers =
      (await page
        .locator('[data-testid="member-list"], .space-y-4:has(a[href^="/members/"])')
        .count()) > 0;
    ...
    await expect(page.locator('h2:has-text("Add Family"), h3:has-text("Add Family")')).toBeVisible({
      timeout: 5000,
    });
    ...
    // Fill in family form
    await page.fill(
      'input[name="family_name"], input[placeholder*="Family name"]',
      familyData.familyName
    );
    await page.fill(
      'input[name="primary_contact_name"], input[placeholder*="Contact name"]',
      familyData.primaryContactName
    );
    await page.fill(
      'input[name="primary_contact_email"], input[type="email"]',
      familyData.primaryContactEmail
    );
    await page.fill(
      'input[name="primary_contact_phone"], input[type="tel"]',
      familyData.primaryContactPhone
    );

    // Fill address fields if visible
    const addressField = page.locator(
      'input[name="address_line_1"], input[placeholder*="Address"]'
    );
    ...
    // Submit family form
    await page.click(
      'button[type="submit"]:has-text("Save"), button:has-text("Create Family"), button:has-text("Add Family")'
    );
    ...
    // Wait for modal to open
    await expect(
      page.locator('h2:has-text("Add Gymnast"), h3:has-text("Add Gymnast")')
    ...
    // Fill in member form
    await page.fill(
      'input[name="first_name"], input[placeholder*="First name"]',
      memberData.firstName
    );
    await page.fill(
      'input[name="last_name"], input[placeholder*="Last name"]',
      memberData.lastName
    );
    ...
    await expect(page.locator(`text=${memberData.firstName} ${memberData.lastName}`)).toBeVisible({
      timeout: 10000,
    });
    ...
    if (sessionOptions > 1) {
      ...
      const hasRoster = await page
        .locator(
          '[data-testid="attendance-roster"], .space-y-2:has(button[aria-label*="attendance"])'
        )
      ...
      if (hasRoster) {
        ...
        const firstAttendanceButton = page
          .locator('button:has-text("Present"), button[aria-label*="Present"]')
          .first();
    ...
    // Check for key page elements
    await expect(
      page.locator('button:has-text("Create Invoice"), button:has-text("Add Invoice")')
    ).toBeVisible();
    ...
    const hasInvoices =
      (await page
        .locator('[data-testid="invoice-list"], .space-y-3:has(div[class*="cursor-pointer"])')
        .count()) > 0;
    const hasInvoiceEmptyState = await page
    ...
    if (hasInvoices) {
      ...
      const statusFilter = page.locator(
        'select:has(option:has-text("Paid")), select:has(option:has-text("Pending"))'
      );
  ...
  test('Login page - UI elements and validation', async ({ page }) => {
    ...
    await expect(
      page.locator('a:has-text("Create an account"), a:has-text("register")')
    ).toBeVisible();
  ...
  test('Login - Invalid credentials', async ({ page }) => {
    ...
    await expect(
      page.locator('text=/invalid.*password/i, text=/error/i, [class*="bg-red"]')
    ).toBeVisible({ timeout: 5000 });
  });
  ...
  test('Members page - Search and filter functionality', async ({ page }) => {
    ...
    await expect(
      page.locator('select:has(option:has-text("Squad")), select[name="squad"]')
    ).toBeVisible();
  });
  ...
  test('Dashboard - Stats and layout', async ({ page }) => {
    ...
    await expect(page.locator('text=/Total|Gymnasts|Active|Revenue/i')).toBeVisible({
      timeout: 5000,
    });
  });
  ...
  test('Squads page - List and navigation', async ({ page }) => {
    ...
    await expect(
      page.locator('button:has-text("Add Squad"), button:has-text("Create Squad")')
    ).toBeVisible();
  });
