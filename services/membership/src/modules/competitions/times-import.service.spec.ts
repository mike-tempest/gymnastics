...
describe('TimesImportService', () => {
  ...
  const mockMembers = [
    ...
    // Two members sharing a name, to exercise the ambiguity error.
    { member_id: 'member-3', registration_number: '1234569', first_name: 'Sam', last_name: 'Lee' },
    { member_id: 'member-4', registration_number: '1234570', first_name: 'Sam', last_name: 'Lee' },
  ];
  ...
  it('imports a sheet with multiple members and multiple times each', async () => {
    const csv = [
    ...
    // PBs recomputed once per distinct member.
    expect(mockPersonalBests.recomputeForMember).toHaveBeenCalledTimes(2);
  });
  ...
  it('reports row-level errors and imports the valid rows', async () => {
    ...
    expect(outcome.errors[1].message).toContain('More than one member');
