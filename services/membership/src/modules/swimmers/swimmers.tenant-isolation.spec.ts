...
/**
 * Cross-tenant isolation test for the swimmers module (Group A pilot).
 *
 * This is the reference pattern the other Phase 3 fan-out agents copy. It drives
 * the real SwimmersRepository through the real TenantScopedHelper /
...
describe('SwimmersRepository tenant isolation', () => {
  let repo: SwimmersRepository;
  ...
  const rows: Array<Partial<Swimmer>> = [
    { swimmer_id: SWIMMER_IN_A, first_name: 'Alice', club_id: CLUB_A },
    { swimmer_id: SWIMMER_IN_B, first_name: 'Bob', club_id: CLUB_B },
  ];

  function matches(row: Partial<Swimmer>, where: ObjectLiteral): boolean {
    return Object.entries(where).every(([key, value]) => row[key as keyof Swimmer] === value);
  }
  ...
  beforeEach(async () => {
    ...
    const fakeTypeOrmRepo = {
      ...
      save: jest.fn((entity: ObjectLiteral) => {
        ...
        return Promise.resolve({ swimmer_id: 'new-id', ...entity });
      }),
    ...
    } as unknown as Repository<Swimmer>;
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: getRepositoryToken(Swimmer), useValue: fakeTypeOrmRepo },
      ],
    }).compile();

    repo = module.get(SwimmersRepository);
  });
  ...
  describe('reads are scoped to the active club', () => {
    it('findAll injects club_id and returns only this club rows', async () => {
      ...
      expect(result[0].swimmer_id).toBe(SWIMMER_IN_A);
    });
  ...
  describe('findOne behaves as not-found across tenants', () => {
    it('returns the row when it belongs to the active club', async () => {
      ...
      const result = await repo.findOne(SWIMMER_IN_A);

      expect(findOneCalls[0]).toEqual({ swimmer_id: SWIMMER_IN_A, club_id: CLUB_A });
      expect(result?.swimmer_id).toBe(SWIMMER_IN_A);
    });

    it('returns null for an id that belongs to another club', async () => {
      cls.set(CLS_CLUB_ID_KEY, CLUB_A);

      // SWIMMER_IN_B exists, but not in CLUB_A, so the scoped lookup is null.
      const result = await repo.findOne(SWIMMER_IN_B);

      expect(findOneCalls[0]).toEqual({ swimmer_id: SWIMMER_IN_B, club_id: CLUB_A });
  ...
  describe('update and delete scope the affected-row predicate by club_id', () => {
    it('update includes club_id in the criteria and drops a supplied club_id', async () => {
      ...
      await repo.update(SWIMMER_IN_A, {
        first_name: 'Renamed',
        club_id: CLUB_B,
      } as CreateSwimmerDto);

      expect(updateCalls[0].criteria).toEqual({ swimmer_id: SWIMMER_IN_A, club_id: CLUB_A });
    ...
    it('delete includes club_id in the criteria', async () => {
      ...
      await repo.remove(SWIMMER_IN_A);

      expect(deleteCalls[0]).toEqual({ swimmer_id: SWIMMER_IN_A, club_id: CLUB_A });
    });
  });
});
