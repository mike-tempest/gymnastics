...
describe('AttendanceController', () => {
  ...
  describe('create', () => {
    it('should call attendanceService.create and return the result', async () => {
      const createDto = {
        ...
        member_id: mockAttendance.member_id,
        status: AttendanceStatus.PRESENT,
      };
  ...
  describe('checkIn', () => {
    it('should call attendanceService.checkInMember and return the result', async () => {
      const body = {
        session_id: mockAttendance.session_id,
        member_id: mockAttendance.member_id,
      };

      mockAttendanceService.checkInMember.mockResolvedValue({
        ...mockAttendance,
      ...
      expect(mockAttendanceService.checkInMember).toHaveBeenCalledWith(
        body.session_id,
        body.member_id,
      );
    });
  ...
  describe('getMemberAttendance', () => {
    it('should return attendance records for a specific member', async () => {
      mockAttendanceService.getMemberAttendance.mockResolvedValue([mockAttendance]);

      const result = await controller.getMemberAttendance(mockAttendance.member_id);

      expect(result).toEqual([mockAttendance]);
      expect(mockAttendanceService.getMemberAttendance).toHaveBeenCalledWith(
        mockAttendance.member_id,
      );
    });
  });
  ...
  describe('getMemberStats', () => {
    it('should return attendance statistics for a member', async () => {
      ...
      mockAttendanceService.getMemberAttendanceStats.mockResolvedValue(
        mockStats as unknown as AttendanceStats,
      );

      const result = await controller.getMemberStats(mockAttendance.member_id);
